  ...
  class GlobaleEva < Globale
   delegate :moyennes_metriques, :ecarts_types_metriques,
    ...
    def persiste
      ...
      @evaluation.update interpretations.merge(redactions: redactions)
      persiste_completude
    end

    def persiste_completude
      completude = Restitution::Eva::Completude.new(evaluation, restitutions).calcule
      @evaluation.update_column(:completude, completude)
    end
    ...
    def moyenne_competences
      @restitutions.each_with_object({}) do |restitution, memo|
        restitution.competences.each do |competence, niveau|
          next if niveau == ::Competence::NIVEAU_INDETERMINE ||
