...
describe PasswordValidator do
  describe "#validate" do
    ...
    it "ne valide rien si le mot de passe est vide" do
      compte = Compte.new(password: "", role: :conseiller)
      described_class.new.validate(compte)
      expect(compte.errors.include?(:password)).to be false
    end

    it "interdit d'utiliser son identifiant comme mot de passe" do
      compte = Compte.new(email: " etienne@ut7.fr", password: "Etienne@ut7.fr ", role: :conseiller)
      described_class.new.validate(compte)
      expect(compte.errors.include?(:password)).to be true
      expect(compte.errors[:password])
        .to eq [ "Votre mot de passe ne doit pas être identique à votre adresse email." ]
    end
  end
end
