...
class Question
  class Import < ::ImportExport::ImportXls
    ...
    def cree_question(row)
      ...
      question.save!
      cree_transcription(:intitule, row[4], row[3], question)
      cree_transcription(:modalite_reponse, row[6], row[5], question) unless question.sous_consigne?
      update_champs_specifiques(row, question)
      question
    end
    # rubocop:enable Metrics/AbcSize

    def intialise_question(row)
    ...
    def update_champs_specifiques(row, question)
      ...
      send(updates[@type], row, question) if updates.key?(@type)
    end

    def update_clic_dans_image(row, question)
      attache_fichier(question.image_au_clic, row[9])
      attache_fichier(question.zone_cliquable, row[8])
    end

    def update_glisser_deposer(row, question)
      cree_reponses('reponse', method(:cree_reponse), row, question)
      attache_fichier(question.zone_depot, row[8])
    end

    def update_qcm(row, question)
      question.update!(type_qcm: row[8])
      cree_reponses('choix', method(:cree_chaque_choix), row, question)
    end

    def update_saisie(row, question)
      question.update!(suffix_reponse: row[8], reponse_placeholder: row[9],
                       type_saisie: row[10])
      cree_reponse_generique(row[11], row[12], 'bon', question)
    end

    def cree_reponses(type, creation_method, row, question)
      extrait_colonnes_reponses(type, row).each_value do |data|
        creation_method.call(data, question)
      end
    end

    def cree_chaque_choix(data, question)
      choix = cree_reponse_generique(data['intitule'], data['nom_technique'], data['type_choix'],
                                     question)
      attache_fichier(choix.audio, data['audio'])
    end

    def cree_reponse(data, question)
      reponse = cree_reponse_generique(nil, data['nom_technique'], data['type_choix'], question,
                                       data['position_client'])
      attache_fichier(reponse.illustration, data['illustration'])
    end

    def cree_reponse_generique(intitule, nom_technique, type_choix, question, position_client = nil)
    ...
    def extrait_colonnes_reponses(reponse, row)
